...
import { MARCAS, sugerirMarcas, sugerirModelos, sugerirComponentes, claveAgrupacion } from '../lib/marcas';
...
// ── AUTOCOMPLETE INPUT ────────────────────────────────────────
function AutoInput({ label, value, onChange, sugerir, placeholder, onSelect }) {
  const [open, setOpen]   = useState(false);
  const [lista, setLista] = useState([]);
  const ref               = useRef(null);

  useEffect(() => {
    setLista(sugerir(value));
    setOpen(true);
  }, [value]);
  ...
  return (
    <div ref={ref} style={{marginBottom:12,position:'relative'}}>
      ...
      />
      {open && lista.length > 0 && (
        <div style={{position:'absolute',top:'calc(100% + 2px)',left:0,right:0,background:'#fff',border:'1px solid #e2dfd8',borderRadius:9,boxShadow:'0 6px 20px rgba(0,0,0,.1)',zIndex:500,maxHeight:200,overflowY:'auto'}}>
          {lista.map(item => (
            <div key={item}
              onMouseDown={() => { onSelect?.(item) || onChange(item); setOpen(false); }}
        ...
        <div style={{padding:'20px 24px',borderBottom:'1px solid #e2dfd8',display:'flex',alignItems:'center',justifyContent:'space-between'}}>
          <div>
            ...
            <div style={{fontSize:12,color:'#9c9a92',marginTop:2}}>
              {pedido.marca} {pedido.modelo} · {tiempoRelativo(pedido.created_at)}
            </div>
          </div>
...
// ── MODAL NUEVO PEDIDO (con autocompletado) ───────────────────
function ModalNuevoPedido({ onClose, onCrear }) {
  const { perfil }                   = useAuth();
  const [form, setForm]              = useState({repuesto:'',marca:'',modelo:'',tipo:'OEM',cantidad:1,prioridad:'normal',observaciones:''});
  const [guardando, setGuardando]    = useState(false);
  const [error, setError]            = useState('');
  ...
  const crear = async () => {
    if (!form.repuesto.trim()) { setError('El nombre/componente del repuesto es obligatorio'); return; }
    setGuardando(true); setError('');
    try {
  ...
  return (
    <div style={{position:'fixed',inset:0,background:'rgba(0,0,0,.4)',zIndex:300,display:'flex',alignItems:'center',justifyContent:'center',padding:16}} onClick={onClose}>
      <div style={{background:'#fff',borderRadius:16,width:'100%',maxWidth:500,maxHeight:'92vh',overflowY:'auto',padding:28}} onClick={e=>e.stopPropagation()}>
        ...
        <AutoInput
          label="MARCA"
          value={form.marca}
          onChange={v => { set('marca',v); set('modelo',''); }}
          sugerir={sugerirMarcas}
          placeholder="Ej: Samsung, iPhone, Xiaomi…"
        />

        {/* Modelo con autocompletado según la marca */}
        <AutoInput
          label="MODELO"
          value={form.modelo}
          onChange={v => set('modelo',v)}
          sugerir={v => sugerirModelos(form.marca, v)}
          placeholder={form.marca ? `Modelos de ${form.marca}…` : 'Primero selecciona una marca'}
        />

        {/* Componente/Repuesto con autocompletado */}
        <AutoInput
          label="COMPONENTE / REPUESTO *"
          value={form.repuesto}
          onChange={v => set('repuesto',v)}
          sugerir={sugerirComponentes}
          placeholder="Ej: Pantalla completa, Batería…"
        />

        {/* Preview de la referencia completa */}
        {(form.marca || form.modelo || form.repuesto) && (
          <div style={{padding:'8px 12px',background:'#f0fdf4',border:'1px solid #bbf7d0',borderRadius:8,fontSize:12.5,color:'#15803d',marginBottom:12,fontWeight:500}}>
            📦 Referencia: <strong>{[form.marca,form.modelo,form.repuesto].filter(Boolean).join(' › ')}</strong>
          </div>
        )}
          ...
          <div style={{display:'flex',gap:8}}>
            {Object.entries(PRIORIDADES).map(([k,v])=>(
              <button key={k} onClick={()=>set('prioridad',k)}
                style={{flex:1,padding:'7px 0',borderRadius:8,border:`1.5px solid ${form.prioridad===k?v.color:'#e2dfd8'}`,background:form.prioridad===k?v.bg:'#fff',color:v.color,fontSize:12.5,fontWeight:600,cursor:'pointer',fontFamily:'inherit'}}>
                {v.label}
              </button>
            ))}
          </div>
        ...
        <div style={{marginBottom:16}}>
          ...
          <textarea value={form.observaciones} onChange={e=>set('observaciones',e.target.value)}
            rows={3} placeholder="Descripción del daño, urgencia, referencia adicional…"
...
// ── VISTA POR TÉCNICO ─────────────────────────────────────────
function VistaTecnicos({ pedidos }) {
  ...
  const porTecnico = useMemo(() => {
    ...
    pedidos.forEach(p => {
      ...
      const id     = p.tecnico_id || 'sin';
      if (!map[id]) map[id] = { id, nombre, avatar:p.tecnico?.avatar||'?', color:p.tecnico?.color||'#e2dfd8', pedidos:[], total:0, entregados:0, pendientes:0 };
      map[id].pedidos.push(p);
      if (p.estado==='entregado') map[id].total += (p.unitario||0)*(p.cantidad||0);
      if (p.estado==='pendiente') map[id].pendientes++;
      if (p.estado==='entregado') map[id].entregados++;
    });
    return Object.values(map).sort((a,b) => b.total - a.total);
  }, [pedidos]);

  const totalGlobal = pedidos.filter(p=>p.estado==='entregado').reduce((a,p)=>a+(p.unitario||0)*(p.cantidad||0),0);

  const tecSel = tecnicoSel ? porTecnico.find(t=>t.id===tecnicoSel) : null;

  return (
    <div>
      {/* Resumen global */}
      <div style={{background:'#1a1916',borderRadius:12,padding:'16px 20px',marginBottom:16,display:'flex',alignItems:'center',justifyContent:'space-between',flexWrap:'wrap',gap:10}}>
        <div>
          <div style={{fontSize:11,fontWeight:700,color:'#9c9a92',letterSpacing:'.5px'}}>TOTAL GLOBAL — TODOS LOS TÉCNICOS</div>
          <div style={{fontSize:26,fontWeight:700,color:'#fff',fontFamily:'monospace',marginTop:2}}>{fmtCOP(totalGlobal)}</div>
        </div>
        <div style={{display:'flex',gap:16}}>
          {[
            {label:'Pedidos',   val:pedidos.length,                                    c:'#dbeafe'},
            {label:'Entregados',val:pedidos.filter(p=>p.estado==='entregado').length,  c:'#dcfce7'},
            {label:'Pendientes',val:pedidos.filter(p=>p.estado==='pendiente').length,  c:'#fef3c7'},
          ].map(m=>(
            <div key={m.label} style={{textAlign:'center'}}>
              <div style={{fontSize:20,fontWeight:700,color:m.c}}>{m.val}</div>
              <div style={{fontSize:10,color:'#9c9a92'}}>{m.label}</div>
            </div>
          ))}
        </div>
      </div>
      ...
      <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fill,minmax(220px,1fr))',gap:12,marginBottom:20}}>
        {porTecnico.map(t => {
          const isActive = tecnicoSel===t.id;
          const pct      = totalGlobal > 0 ? (t.total/totalGlobal)*100 : 0;
          return (
            <div key={t.id}
              onClick={() => setTecnicoSel(isActive?null:t.id)}
              style={{background:'#fff',border:`2px solid ${isActive?'#1a1916':'#e2dfd8'}`,borderRadius:12,padding:'14px 16px',cursor:'pointer',transition:'all .15s'}}>
              <div style={{display:'flex',alignItems:'center',gap:10,marginBottom:12}}>
                <div style={{width:36,height:36,borderRadius:'50%',background:t.color,display:'flex',alignItems:'center',justifyContent:'center',fontSize:13,fontWeight:700,color:'#fff',flexShrink:0}}>
                  {t.avatar}
                </div>
                <div style={{flex:1,minWidth:0}}>
                  <div style={{fontSize:13.5,fontWeight:600,overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap'}}>{t.nombre}</div>
                  <div style={{fontSize:11,color:'#9c9a92'}}>{t.pedidos.length} pedidos</div>
                </div>
                {isActive && <span style={{fontSize:12,color:'#1a1916'}}>✓</span>}
              </div>
              <div style={{fontSize:19,fontWeight:700,fontFamily:'monospace',marginBottom:8,color:'#15803d'}}>{fmtCOP(t.total)}</div>
              {/* Barra de proporción vs el global */}
              <div style={{height:4,background:'#f0eee9',borderRadius:99,overflow:'hidden',marginBottom:8}}>
                <div style={{height:'100%',background:'#2563eb',borderRadius:99,width:pct+'%',transition:'width .4s'}}/>
              </div>
              <div style={{display:'flex',gap:8}}>
                {[
                  {label:'Entregados',val:t.entregados,c:'#15803d',bg:'#dcfce7'},
                  {label:'Pendientes',val:t.pendientes,c:'#d97706',bg:'#fef3c7'},
                ].map(m=>(
                  <span key={m.label} style={{flex:1,textAlign:'center',padding:'3px 0',borderRadius:6,background:m.bg,fontSize:11,fontWeight:600,color:m.c}}>
                    {m.val} {m.label}
                  </span>
                ))}
              </div>
      ...
      {/* Pedidos del técnico seleccionado */}
      {tecSel && (
        <div>
          <div style={{fontSize:14,fontWeight:600,marginBottom:10,display:'flex',alignItems:'center',gap:8}}>
            <div style={{width:22,height:22,borderRadius:'50%',background:tecSel.color,display:'flex',alignItems:'center',justifyContent:'center',fontSize:9,fontWeight:700,color:'#fff'}}>{tecSel.avatar}</div>
            Pedidos de {tecSel.nombre}
            <span style={{fontSize:12,color:'#9c9a92',fontWeight:400}}>· {tecSel.pedidos.length} registros</span>
          </div>
          <ListaPedidosCompacta pedidos={tecSel.pedidos}/>
        </div>
      )}
    </div>
  );
...
function VistaAgrupada({ pedidos }) {
  ...
  const grupos = useMemo(() => {
    const map = {};
    pedidos.forEach(p => {
      const clave = claveAgrupacion(p.marca, p.modelo, p.repuesto);
      ...
      map[clave].pedidos.push(p);
      if (p.estado==='entregado') map[clave].total += (p.unitario||0)*(p.cantidad||0);
      if (p.estado==='pendiente') map[clave].totalPend++;
    });
  ...
  return (
    <div>
      <div style={{fontSize:12,color:'#6b6860',marginBottom:12}}>
        {grupos.length} referencias únicas · {pedidos.length} pedidos totales
      </div>
      <div style={{display:'flex',flexDirection:'column',gap:8}}>
        {grupos.map(g => {
          const isOpen = expandido===g.clave;
          const componentes = [...new Set(g.pedidos.map(p=>p.repuesto))];
          const tecnicos    = [...new Set(g.pedidos.map(p=>p.tecnico?.nombre).filter(Boolean))];
          const estados     = g.pedidos.reduce((acc,p)=>{ acc[p.estado]=(acc[p.estado]||0)+1; return acc; },{});
          return (
            <div key={g.clave} style={{background:'#fff',border:`1px solid ${isOpen?'#1a1916':'#e2dfd8'}`,borderRadius:12,overflow:'hidden',transition:'border .15s'}}>
              {/* Header del grupo */}
              <div onClick={()=>setExpandido(isOpen?null:g.clave)}
                style={{display:'flex',alignItems:'center',gap:12,padding:'12px 16px',cursor:'pointer'}}
                onMouseEnter={e=>e.currentTarget.style.background='#fafaf9'}
                onMouseLeave={e=>e.currentTarget.style.background='#fff'}>
                <div style={{width:38,height:38,background:'#f0eee9',borderRadius:9,display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center',flexShrink:0}}>
                  <span style={{fontSize:9,fontWeight:700,color:'#9c9a92'}}>×{g.pedidos.length}</span>
                ...
                <div style={{flex:1,minWidth:0}}>
                  ...
                  <div style={{fontSize:11.5,color:'#9c9a92',marginTop:2,overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap'}}>
                    {tecnicos.join(' · ')} · {componentes.length} componente{componentes.length>1?'s':''}
                  </div>
                </div>
                <div style={{display:'flex',gap:6,alignItems:'center',flexShrink:0,flexWrap:'wrap',justifyContent:'flex-end'}}>
                  {Object.entries(estados).map(([est,cnt])=>(
                    <Badge key={est} cfg={{...ESTADOS[est], label:`${cnt} ${ESTADOS[est]?.label||est}`}}/>
                  ))}
                  {g.total>0 && <span style={{fontSize:12,fontFamily:'monospace',color:'#15803d',fontWeight:700}}>{fmtCOP(g.total)}</span>}
              ...
              {/* Detalle del grupo */}
              {isOpen && (
                <div style={{borderTop:'1px solid #f0eee9',background:'#fafaf9'}}>
                  <ListaPedidosCompacta pedidos={g.pedidos}/>
                </div>
              )}
            </div>
          );
...
// ── LISTA COMPACTA (usada en vistas técnico y agrupada) ───────
function ListaPedidosCompacta({ pedidos }) {
  return (
    <div style={{display:'flex',flexDirection:'column'}}>
      {pedidos.map((p,i) => (
        <div key={p.id} style={{display:'flex',alignItems:'center',gap:10,padding:'10px 16px',borderBottom:i<pedidos.length-1?'1px solid #f0eee9':'none',fontSize:13}}>
          <span style={{fontFamily:'monospace',fontSize:11,color:'#9c9a92',flexShrink:0}}>{fmtNumPedido(p.numero)}</span>
          <div style={{flex:1,minWidth:0}}>
            <div style={{fontWeight:500,overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap'}}>{p.repuesto}</div>
            <div style={{fontSize:11,color:'#9c9a92'}}>{p.tecnico?.nombre} · {tiempoRelativo(p.created_at)}</div>
          </div>
          <Badge cfg={PRIORIDADES[p.prioridad]||PRIORIDADES.normal}/>
          <Badge cfg={ESTADOS[p.estado]||ESTADOS.pendiente}/>
          <span style={{fontFamily:'monospace',fontSize:12,color:p.estado==='entregado'?'#15803d':'#9c9a92',flexShrink:0}}>
            {p.unitario ? fmtCOP((p.unitario||0)*(p.cantidad||0)) : '—'}
          </span>
        </div>
      ))}
    </div>
